...
/*
 * outputs/test_spmp_ok_u0_rw1_x0_s1_sum0_match0.c
...
void smode_main() {
	/* configure sPMP */
   	...
   	asm volatile ("csrw 0x1b1, %0 \n" :: "r"((TEST_MEM_START + 256) >> 2) : "memory"); //spmpaddr1
	...
	reg_t cfg1 = (PMP_R | PMP_W | PMP_X | SPMP_S | PMP_TOR);   // for S_MEM
...
#if 1
    	cfg0 |= ((1 ? (PMP_R | PMP_W)  : 0) | (0 ? PMP_X : 0) | PMP_TOR | (1 ? SPMP_S : 0)) << 16;
#endif   
   	...
   	asm volatile ("csrw 0x1b0, %0 \n" :: "r"((TEST_MEM_START + 256) >> 2) : "memory"); //spmpaddr0
...
#if 1
    	cfg0 |= ((1 ? (PMP_R | PMP_W)  : 0) | (0 ? PMP_X : 0) | PMP_TOR | (1 ? SPMP_S : 0)) << 8;
#endif   
    	asm volatile ("csrw 0x1a0, %0 \n"
    	            :
    	            : "r"(cfg0)
